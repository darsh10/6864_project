...
class Model:
        ...
        for epoch in xrange(max_epoch):
            ...
            for i in xrange(N):
                ...
                if i == N-1:
                    ...
                    if dev is not None:
                        dev_MAP, dev_MRR, dev_P1, dev_P5 = self.evaluate(dev, eval_func, 'evaluate_dev.txt')
                    if test is not None:
                        test_MAP, test_MRR, test_P1, test_P5 = self.evaluate(test, eval_func, args.test+'_evaluate')

                    if dev is None and test is not None:
                        result_table.add_row(                                                                   [ epoch ] +                                                                         [ "%.2f" % x for x in [ 0.0, 0.0, 0.0, 0.0 ] +
    ...
    def evaluate(self, data, eval_func, evaluation_file):
        results_file = open(evaluation_file,'a')
        res = [ ]
        result_labels = []
        result_scores = []
        for idts, idbs, labels in data:
            ...
            result_labels.append([ranks])
            result_scores.append([scores])
            #result_labels.append([ranked_labels])
        e = Evaluation(res)
        MAP = e.MAP()*100
        MRR = e.MRR()*100
